    ...
    private IReadOnlyList<string> GetSchemaStatements()
    {
        if (provider == RelationalDatabaseProvider.SqlServer)
        {
            ...
            [
                ...
                "IF OBJECT_ID('OrderAuditLogs', 'U') IS NULL CREATE TABLE OrderAuditLogs (Id NVARCHAR(40) NOT NULL PRIMARY KEY, OrderId INT NOT NULL, OrderNumber NVARCHAR(100) NOT NULL, Operation NVARCHAR(50) NOT NULL, PreviousStatus NVARCHAR(20) NOT NULL, NewStatus NVARCHAR(20) NOT NULL, Reason NVARCHAR(500) NOT NULL, Actor NVARCHAR(100) NOT NULL, CreatedAt DATETIME2 NOT NULL, CONSTRAINT FK_OrderAuditLogs_Orders FOREIGN KEY (OrderId) REFERENCES CustomerOrders(Id));",
                "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_OrderAuditLogs_OrderNumber_CreatedAt' AND object_id = OBJECT_ID('OrderAuditLogs')) CREATE INDEX IX_OrderAuditLogs_OrderNumber_CreatedAt ON OrderAuditLogs (OrderNumber, CreatedAt DESC);"
            ];
        }
        ...
        [
            ...
            "CREATE TABLE IF NOT EXISTS OrderAuditLogs (Id TEXT NOT NULL PRIMARY KEY, OrderId INTEGER NOT NULL, OrderNumber TEXT NOT NULL, Operation TEXT NOT NULL, PreviousStatus TEXT NOT NULL, NewStatus TEXT NOT NULL, Reason TEXT NOT NULL, Actor TEXT NOT NULL, CreatedAt TEXT NOT NULL, FOREIGN KEY (OrderId) REFERENCES CustomerOrders(Id));",
            "CREATE INDEX IF NOT EXISTS IX_OrderAuditLogs_OrderNumber_CreatedAt ON OrderAuditLogs (OrderNumber, CreatedAt DESC);"
        ];
    }
}
